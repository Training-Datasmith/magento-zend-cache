 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zend_Cache_Backend_Memcached extends Zend_Cache_Backend implements Zend_cache_backend_extended_Interface
{
    /**
     * Default Values
     */
     ...
     */
    protected $_options = ['servers' => [['host' => self::DEFAULT_HOST, 'port' => self::DEFAULT_PORT, 'persistent' => self::DEFAULT_PERSISTENT, 'weight' => self::DEFAULT_WEIGHT, 'timeout' => self::DEFAULT_TIMEOUT, 'retry_interval' => self::DEFAULT_RETRY_INTERVAL, 'status' => self::DEFAULT_STATUS, 'failure_callback' => self::DEFAULT_FAILURE_CALLBACK]], 'compression' => false, 'compatibility' => false];
    /**
     * Memcache object
     *
    ...
    public function __construct(array $options = [])
    {
        if (!extension_loaded('memcache')) {
            Zend_Cache::throw_exception('The memcache extension must be loaded for using this backend !');
        }
        ...
        foreach ($this->_options['servers'] as $server) {
            ...
            if ($this->_options['compatibility']) {
                ...
                $this->_memcache->add_server($server['host'], $server['port'], $server['persistent'], $server['weight'], $server['timeout'], $server['retry_interval']);
            } else {
                $this->_memcache->add_server($server['host'], $server['port'], $server['persistent'], $server['weight'], $server['timeout'], $server['retry_interval'], $server['status'], $server['failure_callback']);
            }
        }
    }
    /**
    ...
    {
        $mems = $this->_memcache->get_extended_stats();
        $mem_size = null;
        $mem_used = null;
        foreach ($mems as $key => $mem) {
             ...
             */
            $each_used = $mem['bytes'] ?? $mem['mem_used'];
    ...
    public function touch($id, $extra_lifetime)
    {
        ...
        if (is_array($tmp)) {
            ...
            if (!$result = $this->_memcache->replace($id, [$data, time(), $new_lifetime], $flag, $new_lifetime)) {
                return $this->_memcache->set($id, [$data, time(), $new_lifetime], $flag, $new_lifetime);
            }
            return $result;
        }
        return false;

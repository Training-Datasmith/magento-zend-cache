 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zend_cache_backend_win_Cache extends Zend_Cache_Backend implements Zend_cache_backend_extended_Interface
{
    /**
     * Log message
     */
    ...
    public function __construct(array $options = [])
    {
        if (!extension_loaded('wincache')) {
            Zend_Cache::throw_exception('The wincache extension must be loaded for using this backend !');
        }
    ...
    public function get_filling_percentage(): int
    {
        ...
        $mem_size = $mem['memory_total'];
        $mem_used = $mem_size - $mem['memory_free'];
        if ($mem_size == 0) {
            Zend_Cache::throw_exception('can\'t get WinCache memory size');
        }
    ...
    public function touch($id, $extra_lifetime)
    {
        ...
        if (is_array($tmp)) {
            ...
            return wincache_ucache_set($id, [$data, time(), $new_lifetime], $new_lifetime);
        }
        return false;
    }
    /**
